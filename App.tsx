...
import { UserRole, Post, ViewType, UserProfile, FeatureType, Feature, Comment, Message, Institution, OnboardingRequest } from './types';
...
const PostCard: React.FC<{ post: Post, currentUser: UserProfile, onUpdate: () => void, viewMode?: 'DASHBOARD' | 'MARKET' | 'SHOWCASE' }> = ({ post, currentUser, onUpdate, viewMode = 'DASHBOARD' }) => {
  ...
  const canManage = currentUser.role === UserRole.LEAD || currentUser.uid === post.authorId;
  ...
  const handleAssign = (userId: string) => {
    db.assignDeveloper(post.id, userId);
    alert("Developer Assigned to Team! (Step 5 Complete)");
    onUpdate();
  };
      ...
      <div className="flex justify-between items-start mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-slate-400 font-medium">{post.authorRole === UserRole.LEAD ? 'Cohort Lead' : 'Founder'} • {new Date(post.timestamp).toLocaleDateString()}</div>
          </div>
        </div>
        <div className="flex gap-2">
           {getStatusBadge()}
...
const NetworkingView: React.FC<{ currentUser: UserProfile, institutionId: string, onMessage: (userId: string) => void }> = ({ currentUser, institutionId, onMessage }) => {
  ...
  useEffect(() => {
    ...
    setUsers(db.getConnectedUsers(currentUser, institutionId));
...
// ... MessagesView and UserDashboard omitted for brevity (unchanged) ...
...
const AdminDashboard: React.FC<{ institutionId: string }> = ({ institutionId }) => {
  ...
  useEffect(() => {
    setPendingPosts(db.getPendingPosts(institutionId));
    setAllUsers(db.adminGetAllUsers(institutionId));
    setDevelopers(db.getDevelopers(institutionId));
  }, [institutionId]);
...
const UnifiedPortal: React.FC<{ onSelect: (inst: Institution) => void, onSuperAdminLogin: (user: UserProfile) => void }> = ({ onSelect, onSuperAdminLogin }) => {
  ...
  // Partner Form State
  const [partnerForm, setPartnerForm] = useState({ name: '', email: '', adminName: '' });
  ...
  const handleEnter = (e: React.FormEvent) => {
    e.preventDefault();
    if (!code.trim()) return;
    const inst = db.getInstitutionByCode(code);
    if (inst) {
      onSelect(inst);
    } else {
      alert("Invalid Cohort Code. Please check and try again.");
    }
  };

  const handleAdminSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const result = db.loginSuperAdmin(adminPwd);
    if (result.success && result.user) {
        onSuperAdminLogin(result.user);
    } else {
  ...
  const handlePartnerSubmit = (e: React.FormEvent) => {
      e.preventDefault();
      if (partnerForm.name && partnerForm.email && partnerForm.adminName) {
          db.submitOnboardingRequest(partnerForm.name, partnerForm.email, partnerForm.adminName);
          alert("Request Submitted! Our team will review your application.");
          setShowPartnerModal(false);
          setPartnerForm({ name: '', email: '', adminName: '' });
      } else {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 relative overflow-x-hidden overflow-y-auto selection:bg-brand-orange selection:text-white">
       ...
       <div className="relative z-10 w-full max-w-7xl mx-auto flex flex-col p-6 md:p-12 gap-16">
              ...
              <div className="w-full max-w-md mx-auto animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
                 <div className="bg-white p-8 md:p-10 rounded-[2.5rem] shadow-2xl border border-slate-100 relative z-20">
                     <div className="text-center mb-8">
                        ...
                        <p className="text-slate-400 font-bold text-sm">Enter your Cohort Code to begin</p>
                     </div>
                     <form onSubmit={handleEnter} className="space-y-4">
                        <div>
                           <label className="text-xs font-bold text-slate-400 ml-2 mb-2 block uppercase">Cohort Code</label>
                           <input 
                              value={code} 
                              onChange={e => setCode(e.target.value)} 
                              placeholder="e.g. ALPHA" 
                              className="w-full p-4 bg-slate-50 rounded-2xl font-bold outline-none border-2 border-transparent focus:border-brand-blue/30 focus:bg-white transition-all text-center text-xl uppercase tracking-widest"
                              autoFocus
                           />
                        </div>
                        <button type="submit" className="w-full py-4 bg-slate-900 text-white rounded-2xl font-bold hover:bg-slate-800 shadow-xl flex items-center justify-center gap-2 group transition-all">
                           Enter BuildForge <ArrowRight size={20} className="group-hover:translate-x-1 transition-transform"/>
                        </button>
                     </form>
                 </div>
                 
                 <div className="text-center mt-8 space-y-4">
                    <button onClick={() => setShowPartnerModal(true)} className="text-sm font-black text-brand-blue hover:text-blue-600 uppercase tracking-widest flex items-center justify-center gap-2 mx-auto bg-blue-50 px-6 py-3 rounded-xl border border-blue-100 hover:border-blue-200 transition-all">
          ...
          {/* SIMULTANEOUS ENGINE INFO SECTION */}
              ...
              <div className="grid md:grid-cols-3 gap-8 items-center max-w-6xl mx-auto">
                  ...
                  <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100 h-full flex flex-col items-center text-center relative hover:shadow-md transition-shadow">
                      ...
                      <ul className="space-y-4 text-left w-full text-slate-600 font-medium">
                          ...
                          <li className="flex items-start gap-3"><span className="w-1.5 h-1.5 rounded-full bg-purple-500 mt-2 shrink-0"></span>Sprints with transparent tracking</li>
                          <li className="flex items-start gap-3"><span className="w-1.5 h-1.5 rounded-full bg-purple-500 mt-2 shrink-0"></span>Reliable, quality-assured delivery</li>
                      </ul>
                  </div>
                  ...
                  <div className="bg-[#F8FEE7] p-8 rounded-[2rem] shadow-sm border border-slate-100 h-full flex flex-col items-center text-center relative hover:shadow-md transition-shadow">
                      ...
                      <ul className="space-y-4 text-left w-full text-slate-700 font-medium">
                          ...
                          <li className="flex items-start gap-3"><span className="w-1.5 h-1.5 rounded-full bg-slate-800 mt-2 shrink-0"></span>Build a professional portfolio</li>
                          <li className="flex items-start gap-3"><span className="w-1.5 h-1.5 rounded-full bg-slate-800 mt-2 shrink-0"></span>Direct mentorship from tech experts</li>
                      </ul>
                  </div>
              </div>
          </div>
       ...
       {showPartnerModal && (
         <div className="fixed inset-0 z-50 bg-slate-900/80 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-white w-full max-w-md rounded-[2rem] p-8 shadow-2xl relative">
                <button onClick={() => setShowPartnerModal(false)} className="absolute top-4 right-4 p-2 text-slate-400 hover:text-red-500"><XCircle size={20}/></button>
                <div className="text-center mb-6">
                    <Building2 className="w-12 h-12 text-brand-blue mx-auto mb-2"/>
                    <h3 className="text-xl font-black text-slate-800">Board Your Institution</h3>
                    <p className="text-slate-500 text-sm">Join the network as a new cohort.</p>
                </div>
                <form onSubmit={handlePartnerSubmit} className="space-y-4">
                    <input value={partnerForm.name} onChange={e => setPartnerForm({...partnerForm, name: e.target.value})} placeholder="Institution / Startup Name" className="w-full p-3 bg-slate-50 rounded-xl font-bold outline-none"/>
                    <input value={partnerForm.email} onChange={e => setPartnerForm({...partnerForm, email: e.target.value})} placeholder="Official Email" className="w-full p-3 bg-slate-50 rounded-xl font-bold outline-none"/>
                    <input value={partnerForm.adminName} onChange={e => setPartnerForm({...partnerForm, adminName: e.target.value})} placeholder="Lead Admin Name" className="w-full p-3 bg-slate-50 rounded-xl font-bold outline-none"/>
                    
                    <button type="submit" className="w-full py-3 bg-brand-blue text-white rounded-xl font-bold shadow-lg hover:bg-blue-600 mt-2">
                        Submit Request
                    </button>
                </form>
            </div>
...
const App: React.FC = () => {
  ...
  const [superAdminView, setSuperAdminView] = useState<'COHORTS' | 'REQUESTS' | 'MESSAGES'>('COHORTS');
  const [superAdminSelectedInstId, setSuperAdminSelectedInstId] = useState<string | null>(null);
  const [superAdminUsersList, setSuperAdminUsersList] = useState<UserProfile[]>([]);
  ...
  const refreshPosts = () => {
    ...
    if (currentView === ViewType.SPRINT_HUB) {
        setPosts(db.getPosts(currentInstitution.id, 'SPRINT_UPDATE', role, uid));
    }
    else if (currentView === ViewType.DEV_MARKET) {
        // Dev Market now acts as "My Assignments" for devs
        setPosts(db.getPosts(currentInstitution.id, 'OPEN_ROLE', role, uid));
    }
    else if (currentView === ViewType.LAUNCHPAD) {
        setPosts(db.getPosts(currentInstitution.id, 'DELIVERY', role, uid));
    }
  };
  ...
  useEffect(() => {
    if (isSuperAdminMode) {
        if (superAdminView === 'REQUESTS') {
            setOnboardingRequests(db.getOnboardingRequests());
        }
        if (superAdminSelectedInstId) {
           setSuperAdminUsersList(db.superAdminGetInstitutionUsers(superAdminSelectedInstId));
        }
    }
  }, [superAdminSelectedInstId, isSuperAdminMode, superAdminView]);

  const handleSuperAdminToggleBlock = (uid: string) => {
     const updatedUser = db.adminToggleBlockUser(uid);
     if (updatedUser) {
        setSuperAdminUsersList(prev => prev.map(u => u.uid === uid ? updatedUser : u));
     }
  };
  ...
  const handleApproveRequest = (id: string) => {
      ...
      alert("Institution Boarded Successfully!");
  };
  ...
  const handleSubmitAuth = () => {
     ...
     if (registrationRole === 'FOUNDER') {
         ...
         const result = db.loginLead(formData.email, formData.accessKey, currentInstitution.id);
         if (result.user) {
  ...
  const handleLoginExisting = () => {
      // Simple Login for demo (using Email only for Founder/Dev)
      if(!currentInstitution) return;
      const result = db.loginUserByEmail(formData.email, currentInstitution.id);
      if(result.user) {
          setCurrentUser(result.user);
          // Redirect based on role
          if (result.user.role === UserRole.LEAD) {
             setCurrentView(ViewType.ADMIN_DASHBOARD);
  ...
  const handleCreatePost = (e: React.FormEvent) => {
    ...
    if (currentView === ViewType.DEV_MARKET) type = 'OPEN_ROLE'; // Only leads/admins likely doing this now manually?
    ...
    // Only Admin can post certain things in strict mode, but founders post ideas.
        ...
        <div className="p-4 md:p-10 bg-slate-50 min-h-screen">
             <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                ...
                <div className="flex items-center gap-4">
                    <button onClick={() => setSuperAdminView('MESSAGES')} className={`px-4 py-2 rounded-xl font-bold flex items-center gap-2 ${superAdminView === 'MESSAGES' ? 'bg-brand-blue text-white' : 'bg-white text-slate-600 border border-slate-200'}`}>
                        <MessageSquareText size={18}/> Messages
                    </button>
                    <button onClick={() => setIsSuperAdminMode(false)} className="px-4 py-2 bg-slate-200 rounded-xl font-bold text-slate-600 hover:bg-slate-300">Exit Root</button>
                </div>
             </div>
             
             {superAdminView === 'MESSAGES' && currentUser ? (
                 <div className="h-[80vh]">
                     <div className="mb-4 flex gap-2">
                        <button onClick={() => setSuperAdminView('COHORTS')} className="px-4 py-2 bg-white border border-slate-200 text-slate-600 rounded-lg font-bold text-xs">Back to Dashboard</button>
                     </div>
                     <NetworkingView 
                ...
                <>
                    <div className="bg-white p-2 rounded-xl border border-slate-200 inline-flex mb-8">
                        <button onClick={() => setSuperAdminView('COHORTS')} className={`px-6 py-2 rounded-lg font-bold text-sm ${superAdminView === 'COHORTS' ? 'bg-slate-800 text-white' : 'text-slate-500 hover:bg-slate-50'}`}>Active Cohorts</button>
                        <button onClick={() => setSuperAdminView('REQUESTS')} className={`px-6 py-2 rounded-lg font-bold text-sm ${superAdminView === 'REQUESTS' ? 'bg-slate-800 text-white' : 'text-slate-500 hover:bg-slate-50'}`}>
                            Requests {onboardingRequests.length > 0 && <span className="ml-2 bg-brand-orange text-white text-[10px] px-2 py-0.5 rounded-full">{onboardingRequests.length}</span>}
                        </button>
                    </div>

                    {superAdminView === 'COHORTS' && (
                        <div className="grid gap-4 mb-8">
                            {db.getInstitutions().map(inst => (
                                <div key={inst.id} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
                                    <div className="flex justify-between items-center mb-4">
                                        <div className="flex items-center gap-3">
                                            <img src={inst.logo} className="w-10 h-10 object-contain"/>
                                            <div>
                                                <h3 className="font-bold text-lg">{inst.name}</h3>
                                                <p className="text-xs text-slate-400 font-bold">{inst.code}</p>
                                            </div>
                                        </div>
                                        <div className="flex gap-2">
                                            <button onClick={() => setSuperAdminSelectedInstId(inst.id === superAdminSelectedInstId ? null : inst.id)} className={`px-4 py-2 rounded-lg font-bold text-xs ${superAdminSelectedInstId === inst.id ? 'bg-brand-blue text-white' : 'bg-slate-100 text-slate-600'}`}>
                                                {superAdminSelectedInstId === inst.id ? 'Hide Users' : 'Manage Users'}
                                            </button>
                                            <button onClick={() => { db.deleteInstitution(inst.id); window.location.reload(); }} className="px-4 py-2 bg-red-50 text-red-600 rounded-lg font-bold text-xs hover:bg-red-100">Deboard Cohort</button>
                                        </div>
                                    </div>
                                    
                                    {/* Expanded User List for Boarding/Deboarding */}
                                    {superAdminSelectedInstId === inst.id && (
                                        <div className="mt-4 border-t border-slate-100 pt-4 animate-fade-in-up">
                                            <h4 className="font-bold text-sm text-slate-400 uppercase mb-3">Cohort Users (Leads, Founders, Devs)</h4>
                                            <div className="space-y-2">
                                                {superAdminUsersList.length === 0 && <p className="text-xs text-slate-400 italic">No users found.</p>}
                                                {superAdminUsersList.map(u => (
                                                    <div key={u.uid} className="flex justify-between items-center p-3 bg-slate-50 rounded-xl">
                                                        <div className="flex items-center gap-3">
                                                            <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs text-white ${u.role === UserRole.LEAD ? 'bg-slate-800' : 'bg-slate-400'}`}>
                                                                {u.role[0]}
                                                            </div>
                                                            <div>
                                                        ...
                                                        <button onClick={() => handleSuperAdminToggleBlock(u.uid)} className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider ${u.blocked ? 'bg-red-500 text-white' : 'bg-green-100 text-green-700'}`}>
                                                            {u.blocked ? 'Deboarded' : 'Boarded'}
                                                        </button>
                                                    </div>
                                                ))}
                                            </div>
                    ...
                    {superAdminView === 'REQUESTS' && (
                         <div className="grid gap-4">
                             {onboardingRequests.length === 0 && <div className="p-10 bg-white rounded-2xl border border-slate-100 text-center text-slate-400 font-bold">No pending requests.</div>}
                             {onboardingRequests.map(req => (
                                 <div key={req.id} className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex justify-between items-center">
                                     <div>
                                         <h3 className="font-black text-lg text-slate-800">{req.instituteName}</h3>
                                         <div className="text-sm text-slate-500">Contact: <b>{req.contactName}</b> ({req.email})</div>
                                         <div className="text-xs bg-yellow-100 text-yellow-700 font-bold px-2 py-1 rounded inline-block mt-2">PENDING APPROVAL</div>
                                     </div>
                                     <div className="flex gap-2">
                                         <button onClick={() => handleApproveRequest(req.id)} className="px-4 py-2 bg-brand-blue text-white rounded-xl font-bold text-sm shadow-md hover:bg-blue-600">Approve & Board</button>
                                         <button onClick={() => handleRejectRequest(req.id)} className="px-4 py-2 bg-slate-100 text-slate-600 rounded-xl font-bold text-sm hover:bg-slate-200">Reject</button>
                                     </div>
                                 </div>
                             ))}
                         </div>
  ...
  if (!currentInstitution) {
    return <UnifiedPortal onSelect={setCurrentInstitution} onSuperAdminLogin={(user) => { setIsSuperAdminMode(true); setCurrentUser(user); }} />;
  }

  // ... (Rest of Step 2 Registration & Main Dashboard logic same as before)
  // Re-pasting just the modified Registration block to ensure no regression from previous turn if file is replaced entirely.
  
  if (currentInstitution && !currentUser) {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 relative overflow-hidden">
        <CursorBloop />
        <div className="max-w-2xl w-full bg-white rounded-[2.5rem] shadow-xl overflow-hidden border border-slate-100 z-10 relative">
           ...
           <div className="p-8 text-center bg-slate-900 text-white relative overflow-hidden">
               ...
               <img src={currentInstitution.logo} className="w-12 h-12 object-contain mx-auto mb-2 bg-white rounded-xl p-1"/>
               <h1 className="text-2xl font-black">BuildForge — Get Started</h1>
               <p className="opacity-60 text-sm font-bold uppercase tracking-wider">{currentInstitution.name}</p>
           </div>
           ...
           <div className="p-8">
               ...
               {!registrationRole ? (
                   <div className="space-y-4">
                       <button onClick={() => setRegistrationRole('FOUNDER')} className="w-full p-6 rounded-2xl border-2 border-slate-100 hover:border-brand-blue hover:bg-blue-50/50 transition-all flex items-center gap-4 text-left group">
                           <div className="w-12 h-12 rounded-full bg-blue-100 text-brand-blue flex items-center justify-center group-hover:scale-110 transition-transform"><Rocket size={24}/></div>
                           <div>
                               <h3 className="font-black text-slate-800 text-lg">Register as Founder</h3>
                               <p className="text-slate-500 text-sm font-medium">I have a startup idea and need tech support.</p>
                           </div>
                       </button>

                       <button onClick={() => setRegistrationRole('DEVELOPER')} className="w-full p-6 rounded-2xl border-2 border-slate-100 hover:border-brand-orange hover:bg-orange-50/50 transition-all flex items-center gap-4 text-left group">
                           <div className="w-12 h-12 rounded-full bg-orange-100 text-brand-orange flex items-center justify-center group-hover:scale-110 transition-transform"><Code2 size={24}/></div>
                           <div>
                               <h3 className="font-black text-slate-800 text-lg">Register as Developer</h3>
                               <p className="text-slate-500 text-sm font-medium">I want to work on real startup projects.</p>
                           </div>
                       </button>

                       <button onClick={() => setRegistrationRole('LEAD')} className="w-full p-6 rounded-2xl border-2 border-slate-100 hover:border-slate-800 hover:bg-slate-50 transition-all flex items-center gap-4 text-left group">
                           <div className="w-12 h-12 rounded-full bg-slate-200 text-slate-800 flex items-center justify-center group-hover:scale-110 transition-transform"><Shield size={24}/></div>
                           <div>
                               <h3 className="font-black text-slate-800 text-lg">Register as Lead</h3>
                               <p className="text-slate-500 text-sm font-medium">I am a BuildForge team lead who manages projects.</p>
                           </div>
                       </button>
                       
                       <div className="pt-6 border-t border-slate-100 mt-4">
                           <p className="text-center text-sm font-bold text-slate-400 mb-3">Already have an account?</p>
                           <div className="flex gap-2">
                               <input value={formData.email} onChange={e => updateForm('email', e.target.value)} placeholder="Enter Email to Login" className="flex-1 p-3 bg-slate-50 rounded-xl font-bold outline-none border border-transparent focus:border-slate-300"/>
                               <button onClick={handleLoginExisting} className="px-6 py-3 bg-slate-800 text-white rounded-xl font-bold hover:bg-slate-700">Login</button>
                           </div>
                       </div>
                   </div>
               ) : (
                   <div className="animate-fade-in-up">
                       <button onClick={() => setRegistrationRole(null)} className="mb-6 flex items-center gap-2 text-xs font-bold text-slate-400 hover:text-slate-600"><ArrowLeft size={14}/> Back to Roles</button>
                       
                       {/* FOUNDER FORM */}
                       {registrationRole === 'FOUNDER' && (
                           <div className="space-y-4">
                               <h3 className="text-xl font-black text-slate-800 mb-4 flex items-center gap-2"><Rocket className="text-brand-blue"/> Founder Registration</h3>
                               <div className="grid md:grid-cols-2 gap-4">
                                   <input placeholder="Full Name" className="p-3 bg-slate-50 rounded-xl font-bold outline-none" onChange={e => updateForm('name', e.target.value)}/>
                                   <input placeholder="Email" className="p-3 bg-slate-50 rounded-xl font-bold outline-none" onChange={e => updateForm('email', e.target.value)}/>
                                   <input placeholder="Phone" className="p-3 bg-slate-50 rounded-xl font-bold outline-none" onChange={e => updateForm('phone', e.target.value)}/>
                                   <input placeholder="Startup Name" className="p-3 bg-slate-50 rounded-xl font-bold outline-none" onChange={e => updateForm('startupName', e.target.value)}/>
                               </div>
                               <select className="w-full p-3 bg-slate-50 rounded-xl font-bold outline-none text-slate-500" onChange={e => updateForm('stage', e.target.value)}>
                                   <option>Select Stage</option>
                                   <option>Idea Phase</option>
                                   <option>Prototype</option>
                                   <option>MVP</option>
                               </select>
                               <textarea placeholder="Startup Description" rows={2} className="w-full p-3 bg-slate-50 rounded-xl font-medium outline-none resize-none" onChange={e => updateForm('description', e.target.value)}></textarea>
                               <textarea placeholder="What Tech Help Needed?" rows={2} className="w-full p-3 bg-slate-50 rounded-xl font-medium outline-none resize-none" onChange={e => updateForm('techHelp', e.target.value)}></textarea>
                               <div className="grid md:grid-cols-2 gap-4">
                                   <input placeholder="Budget (Optional)" className="p-3 bg-slate-50 rounded-xl font-bold outline-none" onChange={e => updateForm('budget', e.target.value)}/>
                                   <input placeholder="Timeline (e.g. 3 months)" className="p-3 bg-slate-50 rounded-xl font-bold outline-none" onChange={e => updateForm('timeline', e.target.value)}/>
                               </div>
                               <button onClick={handleSubmitAuth} className="w-full py-4 bg-brand-blue text-white rounded-2xl font-bold shadow-lg hover:bg-blue-600 mt-4">Submit Founder Application</button>
                           </div>
                       )}
                       ...
                       {registrationRole === 'DEVELOPER' && (
                           <div className="space-y-4">
                               <h3 className="text-xl font-black text-slate-800 mb-4 flex items-center gap-2"><Code2 className="text-brand-orange"/> Developer Registration</h3>
                               <div className="grid md:grid-cols-2 gap-4">
                                   <input placeholder="Full Name" className="p-3 bg-slate-50 rounded-xl font-bold outline-none" onChange={e => updateForm('name', e.target.value)}/>
                                   <input placeholder="Email" className="p-3 bg-slate-50 rounded-xl font-bold outline-none" onChange={e => updateForm('email', e.target.value)}/>
                                   <input placeholder="Phone" className="p-3 bg-slate-50 rounded-xl font-bold outline-none" onChange={e => updateForm('phone', e.target.value)}/>
                                   <input placeholder="College / Institute" className="p-3 bg-slate-50 rounded-xl font-bold outline-none" onChange={e => updateForm('college', e.target.value)}/>
                               </div>
                               <input placeholder="Key Skills (e.g. React, Python, UI/UX)" className="w-full p-3 bg-slate-50 rounded-xl font-bold outline-none" onChange={e => updateForm('skills', e.target.value)}/>
                               <div className="grid md:grid-cols-2 gap-4">
                                   <input placeholder="GitHub / Portfolio URL" className="p-3 bg-slate-50 rounded-xl font-bold outline-none" onChange={e => updateForm('github', e.target.value)}/>
                                   <input placeholder="Time Availability (hrs/week)" className="p-3 bg-slate-50 rounded-xl font-bold outline-none" onChange={e => updateForm('availability', e.target.value)}/>
                               </div>
                               <textarea placeholder="Experience / Bio" rows={3} className="w-full p-3 bg-slate-50 rounded-xl font-medium outline-none resize-none" onChange={e => updateForm('experience', e.target.value)}></textarea>
                               <button onClick={handleSubmitAuth} className="w-full py-4 bg-brand-orange text-white rounded-2xl font-bold shadow-lg hover:bg-orange-600 mt-4">Submit Developer Profile</button>
                           </div>
                       )}
                       ...
                       {registrationRole === 'LEAD' && (
                           <div className="space-y-4">
                               <h3 className="text-xl font-black text-slate-800 mb-4 flex items-center gap-2"><Shield className="text-slate-700"/> Lead Access</h3>
                               <p className="text-xs font-bold text-slate-400 bg-slate-100 p-3 rounded-xl">Restricted Area: Requires Institution Admin Key</p>
                               <input placeholder="Email" className="w-full p-3 bg-slate-50 rounded-xl font-bold outline-none" onChange={e => updateForm('email', e.target.value)}/>
                               <input type="password" placeholder="Password" className="w-full p-3 bg-slate-50 rounded-xl font-bold outline-none" onChange={e => updateForm('password', e.target.value)}/>
                               <input type="password" placeholder="Admin Access Key" className="w-full p-3 bg-slate-50 rounded-xl font-bold outline-none border-2 border-transparent focus:border-red-200" onChange={e => updateForm('accessKey', e.target.value)}/>
                               
                               <button onClick={handleSubmitAuth} className="w-full py-4 bg-slate-800 text-white rounded-2xl font-bold shadow-lg hover:bg-slate-700 mt-4">Authenticate Lead</button>
                           </div>
                       )}
                   </div>
               )}
           ...
           <div className="bg-slate-50 p-4 text-center">
                <button onClick={handleBackToHome} className="text-xs font-bold text-slate-400 hover:text-slate-600">Back to Home</button>
           </div>
        </div>
      </div>
    );
      ...
      return (
        <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row">
            ...
            <div className="p-8">
                ...
                <p className="text-xs font-bold text-slate-400 mt-1 pl-11">{currentInstitution.code} Cohort</p>
            </div>
            ...
            {/* AI Assistant FAB */}
            <button onClick={() => setActiveFeature({ id: FeatureType.CONTENT_ASSISTANT, title: 'AI Helper', subtitle: '', description: '', icon: '', color: '', bgColor: '' })} className="fixed bottom-20 md:bottom-24 right-6 w-14 h-14 text-white rounded-full shadow-xl flex items-center justify-center hover:scale-110 transition-transform z-40" style={{ backgroundColor: currentInstitution.themeColor }}><Sparkles size={24} /></button>
                ...
                <div className="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4">
                    <div className="bg-white w-full max-w-lg rounded-[2rem] p-8 shadow-2xl">
                        ...
                        <form onSubmit={handleCreatePost} className="space-y-4">
                            ...
                            <button type="submit" className="w-full py-3 text-white rounded-xl font-bold hover:opacity-90" style={{ backgroundColor: currentInstitution.themeColor }}>
